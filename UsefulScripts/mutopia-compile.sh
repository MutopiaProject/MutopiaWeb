...
f_compile_a4() {
   ...
   PAPERSIZE=a4
   $LILYPOND_BIN $PS_AND_PDF -dno-point-and-click -dpaper-size=\"a4\" "$LY_BASE_NAME.ly"
   f_check_papersize
   mv "$LY_BASE_NAME.ps" "$TARGET_BASE_NAME-a4.ps"
   mv "$LY_BASE_NAME.pdf" "$TARGET_BASE_NAME-a4.pdf"
   # svgz files not currently served with the correct mime type by the main Mutopia server
